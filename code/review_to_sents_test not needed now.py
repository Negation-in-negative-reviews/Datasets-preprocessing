...
if __name__ == "__main__":
    ...
    #     myprint(data)
    myprint(f"args: {args}")
    for path in [args.pos_data_path, args.neg_data_path]:
        # myprint(key)
        reviews = utils.read_file(path)
        parent_dir = os.path.dirname(path)
        sents = utils.get_sents(reviews)
        train_size = int(0.9*len(sents))
        split_data_dir = os.path.join(os.path.dirname(path), "split_data")
        Path(split_data_dir).mkdir(parents=True, exist_ok=True)
        sents_filename = os.path.join(split_data_dir, path+"_sents")
        utils.write_file(sents[:train_size], sents_filename)
